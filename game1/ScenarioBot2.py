...
AttackResponse = [" with a wet noodle", "with a Massive Blow", " by tickling its feet", " with a Incredible Upper Cut",
                  " by verbally assualting his opponent",
                  " by creating a thousand spears that stab through their opponent", " with a garbage can",
                  " by flexing",
...
#bot.load_extension("assets.PhoenixDown")
bot.load_extension("assets.cogTest")


@bot.command(pass_context = True)
async def pd(ctx):
    await bot.get_cog("Mycog").downActions()
    await bot.get_cog("Mycog").receiveList(monsters)
...
async def ts(ctx, filename):
    try:
        ...
        if len(PlayerList) < 1:
            await bot.say(
                "There are no pre-defined players. Create players for the scenario with the !create <Playername> command. Enjoy the game!")
        return ScenarioName
    except:
        await bot.say(
            ctx.message.author.mention + " " + filename + " was not found targeted make sure its in the same file path as bot ")
...
async def gameSettings(ctx):
    ...
    if ScenarioName == "":
        await bot.say(
            "There is no Scenario Selected or Loaded. Select a Scenario with the !ts <ScenarioName> command and load it with !loadEnemy command")
        return
...
async def join(ctx, member: discord.Member, playernumber: int = -1):
    ...
    if playernumber > 0:
        ...
        if PlayerList[index].hasID == True:
            await bot.say(
                ctx.message.author.mention + " Somebody has chosen that player already. Choose an available player.")
            return
...
async def unjoin(ctx):
    ...
    if isPlaying == True:
        await bot.send_message(ctx.message.author,
                               "Game is currently in session cant unjoin from game sorry its not implemented yet")
        await bot.delete_message(ctx.message)
        return
    count = 0
    for player in PlayerList:
                    ...
                    if TurnOrder[count].Whoami() == "Enemy":
                        ...
                        if TurnOrder[count].isAlive() == False:
                            ...
                            for enemy in TempEnemyList:
                                if TempEnemyList[countx].isAlive() == False:
                                    tempDead.append(EnemyList[countx])
                        ...
                        if lastindex == -1:
                            ...
                            if damage < 0:
                                ...
                                await bot.say(TurnOrder[count].name + " tried to damage " + TempPlayerList[
                                    0].name + " but ended up healing them for" + str(damage))
                                if count == ResetIndex:
                                    count = 0
                                    continue
                                count += 1
                            ...
                            TempPlayerList[0].Hp = int(TempPlayerList[0].Hp - damage)
                            await bot.say(
                                TurnOrder[count].name + " did " + str(damage) + " damage to " + TempPlayerList[
                                    0].name + choice)
                            if count == ResetIndex:
                                count = 0
                                continue
                        ...
                        if(Down.downInitializer() == True):
                            Down.downActions(self, TempPlayerList,tempDead)
                        ...
                        if damage < 0:
                            ...
                            await bot.say(TurnOrder[count].name + " tried to damage " + TempPlayerList[
                                Targetindex].name + " but ended up healing them for" + str(damage))
                            if count == ResetIndex:
                                count = 0
                                continue
                            count += 1
                        ...
                        TempPlayerList[Targetindex].Hp = int(TempPlayerList[Targetindex].Hp - damage)
                        choice = AttackResponse[random.randint(0, len(AttackResponse) - 1)]
                        await bot.say(TurnOrder[count].name + " did " + str(damage) + " damage to " + TempPlayerList[
                            Targetindex].name + choice)
                        if count == ResetIndex:
                            count = 0
                            continue
                        count += 1
                    ...
                    if TurnOrder[count].Whoami() == "DPlayer":
                        ...
                        if msg.content.upper() == "ATTACK":
                            ...
                            if damage < 0:
                                ...
                                await bot.say(TurnOrder[count].name + " tried to damage " + TempEnemyList[
                                    Targetindex].name + " but ended up healing them for" + str(damage))
                                x = await bot.say(TurnOrder[count].name + ": harder daddy")
                                await bot.delete_message(x)
                                if count == ResetIndex:
                                    count = 0
                                    continue
                            ...
                            await bot.say(TurnOrder[count].name + " did " + str(damage) + " damage to " + TempEnemyList[
                                target].name + choice)
